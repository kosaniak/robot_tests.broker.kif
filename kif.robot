...
Підготувати клієнт для користувача
  ...
  Run Keyword If   '${ARGUMENTS[0]}' != 'kif_Viewer'   Login   ${ARGUMENTS[0]}
...
Завантажити документ
  ...
  ...      ${ARGUMENTS[2]} ==  ${TENDER}
  kif.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}   ${ARGUMENTS[2]}
...
Скасувати закупівлю
  ...
  kif.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]}
...
Отримати інформацію із предмету
  ...
  ${return_value}=  Run Keyword And Return  kif.Отримати інформацію із тендера  ${username}  ${tender_uaid}  ${field_name}
  [return]           ${return_value}
...
Отримати інформацію про dgfDecisionDate
  ...
  ${return_value}=   kif_service.convert_date    ${date_value}
  [return]           ${return_value}
...
Підтвердити підписання контракту
  ...
  Sleep    5
  kif.Завантажити угоду до тендера   ${username}  ${tender_uaid}  1  ${filepath}
  Wait Until Page Contains Element      xpath=(//*[@id='tPosition_status' and not(contains(@style,'display: none'))])
